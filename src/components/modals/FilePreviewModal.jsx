...
export default function FilePreviewModal({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (
        currentObjectUrl &&
        currentObjectUrl.startsWith('blob:')
      ) {
  ...
  const renderPreviewBody = () => {
      ...
      return (
        <div className="text-center py-12 px-4 bg-red-50 rounded border border-red-200">
          ...
          <p className="text-sm text-red-600 mb-4">
            Koneksi terputus atau server lambat.
          </p>

          <div className="flex justify-center gap-2">
            <button
              onClick={() =>
                handleOpenPreview(previewItem)
              }
    ...
    switch (previewItem.type) {
      ...
      case 'text':
        ...
        if (previewUrl) {
          return (
            <div className="w-full h-[65vh] rounded border border-gray-300 overflow-hidden">
              <iframe
                src={previewUrl}
                title={previewItem.name}
        ...
        // Local storage
        return (
          <pre className="p-4 bg-gray-900 text-green-400 font-mono text-xs rounded max-h-[70vh] overflow-auto whitespace-pre-wrap">
            {textContent}
          </pre>
        );
      ...
      case 'video':
        return blobUrl ? (
          <div className="flex justify-center bg-black rounded p-2 min-h-[250px] items-center">
            <video
              controls
              src={blobUrl}
        ...
        return (
          <div className="text-center py-12 px-4 bg-gray-50 rounded border border-gray-200">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 bg-black text-white px-4 py-2 rounded text-sm font-semibold hover:bg-gray-800"
            >
      ...
      <div className="bg-white border-2 border-black p-3.5 sm:p-6 rounded-lg w-full max-w-4xl shadow-2xl relative flex flex-col max-h-[95vh] sm:max-h-[90vh]">
        ...
        <div className="flex items-center justify-between mb-4 pb-2 border-b border-gray-200 gap-2">
          ...
          <div className="flex items-center gap-2 min-w-0 flex-1">
            ...
            <h3
              className="text-sm sm:text-lg font-bold truncate min-w-0"
              title={previewItem.name}
            >
              {previewItem.name}
            </h3>
